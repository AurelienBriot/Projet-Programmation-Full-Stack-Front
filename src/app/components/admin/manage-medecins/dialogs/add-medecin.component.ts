...
export class AddMedecinDialog {
  ...
  newUtilisateur : Utilisateur = { nom: "", prenom: "", adresse: "", ville: "", telephone: "", role: "MEDECIN", email: "", login: "", password: ""};

  centres : Centre[] = [];

  constructor(private utilisateurService: UtilisateurService, public dialogRef: MatDialogRef<AddMedecinDialog>) {
  ...
  submit(form: any) {
    if (form.valid) {
      ...
      this.utilisateurService.addMedecin(this.newUtilisateur).subscribe({
        next: (utilisateur) => {
