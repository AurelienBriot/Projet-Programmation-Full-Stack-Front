...
@Component({
  ...
  export class UpdatePasswordMedecinDialog {
  
    updatedUtilisateur : Utilisateur = { nom: "", prenom: "", adresse: "", ville: "", telephone: "", role: "MEDECIN", email: "", login: "", password: "" };
  
    constructor(private utilisateurService: UtilisateurService, public dialogRef: MatDialogRef<UpdatePasswordMedecinDialog>, @Inject(MAT_DIALOG_DATA) public admin: any) {
    ...
    submit(form: any) {
      if (form.valid) {
        this.utilisateurService.updatePasswordMedecin(this.updatedUtilisateur).subscribe({
          next: (utilisateur) => {
