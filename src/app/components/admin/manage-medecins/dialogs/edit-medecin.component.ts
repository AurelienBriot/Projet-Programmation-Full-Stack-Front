...
@Component({
  ...
  export class UpdateMedecinDialog {
  
    updatedUtilisateur : Utilisateur = { nom: "", prenom: "", adresse: "", ville: "", telephone: "", role: "MEDECIN", email: "", login: "", password: "" };
  
    constructor(private utilisateurService: UtilisateurService, public dialogRef: MatDialogRef<UpdateMedecinDialog>, @Inject(MAT_DIALOG_DATA) public admin: any) {
      this.updatedUtilisateur = { ...admin };
    }
  
    submit(form: any) {
      if (form.valid) {
        this.utilisateurService.updateMedecin(this.updatedUtilisateur).subscribe({
          next: (utilisateur) => {
