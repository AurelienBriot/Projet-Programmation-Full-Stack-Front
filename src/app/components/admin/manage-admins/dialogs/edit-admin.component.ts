...
@Component({
  ...
  export class UpdateAdminDialog {
    ...
    constructor(private utilisateurService: UtilisateurService, public dialogRef: MatDialogRef<UpdateAdminDialog>, @Inject(MAT_DIALOG_DATA) public admin: any) {
      this.updatedUtilisateur = { ...admin };
    }
  
    submit(form: any) {
      if (form.valid) {
        this.utilisateurService.updateAdmin(this.updatedUtilisateur).subscribe({
          next: (utilisateur) => {
